$(() => {
  ...
  //****** label header ******/
  if ((mid > 0) && (roleID == 1)){
    $('#pagetitle').html(`${formatMessage('brandby')} - ${$('#manufacturerName').text()}`);
  } else if ((parentGroupID == 0) && (roleID == 1)) { 
    $('#pagetitle').html(`${formatMessage('brand')}`); 
  } else {
    $('#pagetitle').html(`${formatMessage('authoritytodo')}`);
  }
  //**************************/
  console.log("parentGroupID: " + parentGroupID +', mid: '+ mid )
  var gid = 0;
  var mode = 'new';
  const URL = ''; 
  const tbl = "brand";
  const primekey = "brandID";
