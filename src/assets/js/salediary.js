$(() => {
  ...
  $('#pagetitle').html(`${formatMessage('salediarybystaff')}`);
  //**************************/
...
//************************************************ */
$.post(`/getsalemember`, {  //Get NVSX - Sale
}).done(function (data){
    if (data.status) {
        ...
        staffSource.forEach(obj => {obj.namex = `${obj.name} (${manDivisionSource.filter(f=>f.status==obj.divisionID)[0].statext})` })
        selectedmember = staffSource[0].memberID;
        ...
        $("#searchBox").dxLookup({
            ...
            displayExpr: "namex",
            onValueChanged(a) {
                let selected = staffSource.filter(f=> f.memberID === a.value);
                selectedmember = selected[0].memberID;
                $('#staffimgx').attr('src', `public/${subdir}/${selected[0].imgLink}`);
                grid.refresh();
                removeMedia(); 
...
const memberStore = new DevExpress.data.CustomStore({
    ...
    [
        ...
        return sendRequestARG(`${URL}/salediarybystaff/${selectedmember}`, 'POST', args);
    },
    insert(values) {
        console.log(JSON.stringify(values,false,4));
        values[`staffID`] = selectedmember;
    ...
    store: new DevExpress.data.CustomStore({
      ...
      load: function() {
        return sendRequest(`${URL}/getmember`, 'POST', {  
            roleID: JSON.stringify([4,6]),
            divisionID: JSON.stringify([1]),
            typex: 0
          });
       }
    }) 
  ...
  function sendRequest(url, method = 'GET', data) {
    ...
    $.ajax(url, {
      ...
      xhrFields: { withCredentials: true },
        ...
        if (url == '/getmember') {
            result.forEach(obj => {obj.namex = `${obj.name} (${roleSource.filter(f=>f.status==obj.roleID)[0].statext})` })
        }
        d.resolve(result);
