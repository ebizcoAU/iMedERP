$(() => {
    ...
    $('#pagetitle').html(`${formatMessage('palletsearch')}`);
    //**************************/
    ...
    const memberStore = new DevExpress.data.CustomStore({
      ...
      load(loadOptions) {
        ...
        return sendRequestARG(`${URL}/palletsearchlist`, 'POST', args);
      },
      update(key, values) {
        ...
        { type: 'buttons',
          ...
          { text: formatMessage('download'),
                ...
                onClick: function (e) {
                  let data = e.row.data;
                  if (data.status >= 1) {
                    window.location = `/boxsearch_${data.palletID}`;
                  } else {
