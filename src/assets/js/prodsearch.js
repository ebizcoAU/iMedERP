$(() => {
    ...
    $('#pagetitle').html(`${formatMessage('prodsearch')}`);
    if ($('#boxID').text() > 0) {
      $('#header').addClass('visible');
      $('#productNamex').html(`${formatMessage('product')}: ${$('#productName').text()}`);
      $('#refIDx').html(`${formatMessage('box')} REFID: ${$('#boxRefID').text()}`);
      $('#uuIDx').html(`${formatMessage('box')} UUID: ${$('#uuID').text()}`);
      $('#itemQtyx').html(`${formatMessage('qty')}: ${$('#itemQty').text()} ${formatMessage('product')}`);
    ...
    const memberStore = new DevExpress.data.CustomStore({
      ...
      load(loadOptions) {
        ...
        return sendRequestARG(`${URL}/prodsearchlist/${$('#boxID').text()}`, 'POST', args);
      },
      update(key, values) {
