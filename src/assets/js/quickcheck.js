$(() => {
    ...
    $('#pagetitle').html(`${formatMessage('quickcheck')}`);
    //**************************/
  ...
  const memberStore = new DevExpress.data.CustomStore({
      key: primekey,
      load() {
          return sendRequest(`${URL}/salediarybystaff/0`, 'POST');
      },
      insert(values) {
  ...
  const grid = $('#profileContainer').dxDataGrid({
      ...
      onRowDblClick(e) {
          if ((e.key !== 0) && (e.rowType=='data')) {
              let data = e.data;
              //console.log(data)
              $('#staffimgx').attr('src', `public/${subdir}/${data.imgLink}`);
              $("#searchBox").html(`${data.namex}`);
              var htmlTxt='';
    ...
    function sendRequest(url, method = 'GET', data) {
      ...
      $.ajax(url, {
        ...
        xhrFields: { withCredentials: true },
          ...
          if (url == '/getmember') {
              result.forEach(obj => {obj.namex = `${obj.name} (${rolexSource.filter(f=>f.status==obj.roleID)[0].statext})` })
          }
          if (url == '/salediarybystaff/0') {
