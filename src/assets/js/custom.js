    ...
    var handleMenubar = function() {
        jQuery('.menu-toggler').on('click',function(){
			...
			if (getCookie('scannerMode')=='ON'){
				$('#toggle-scanner-menu').prop('checked', true);
			} else{
				$('#toggle-scanner-menu').prop('checked', false);
			}
		});
		...
		if (!window.matchMedia('(display-mode: standalone)').matches) {
			if (getCookie('pwa-modal')==0) {
				...
				jQuery('.pwa-backdrop, .pwa-close, .pwa-btn').on('click',function(){
					...
					if ($(this).hasClass('.pwa-btn')) setCookie(PwaKey, true)
					else if ($(this).hasClass('.pwa-close')) setCookie(PwaKey, false);
				}); 
	...
	var handleWizardSteps = function (){		
		if(jQuery('#DZWizardSteps').length > 0){			
			$("#DZWizardSteps").steps({
				...
				autoFocus: true,
				onFinishing: function (event, currentIndex) { window.location.href = "/mpaymentconfirm" }, 
			});
		}
	}
...
$(function() {
	...
	if (window.matchMedia("(max-width: 512px)").matches)  { //mobile devices
		...
		setCookie('scannerMode', 'ON'); //desktop devices
		$('#toggle-scanner-menu').prop('checked', true);
	} 

});

function handleMenuLogo(themeColor) { //************ VP **********************************
	if ([
	'color-green',
	'color-blue',
	'color-pink',
	...
	'color-deeporange'
		...
		$('#memulogo').removeClass('text-white');
		$('#memulogo').addClass('text-dark');
	} else if ([
		'color-purple',
		'color-red'
	].includes(themeColor)) {
		$('#memulogo').removeClass('text-dark');
		$('#memulogo').addClass('text-white');
	} else {
		if (getCookie('themeVersion_value')=='theme-dark'){
			$('#memulogo').removeClass('text-dark');
			$('#memulogo').addClass('text-white');
