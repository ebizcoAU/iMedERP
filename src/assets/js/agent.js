...
$(() => {
  var dsid = $('#diststaffID').text();
  //****** label header ******/
  $('#pagetitle').html(`${formatMessage('agent')}`);
  ...
  //**************************/
  var parentGroupID = $('#diststaffgroupID').text();
  ...
  const memberStore = new DevExpress.data.CustomStore({
    key: primekey,
    load() {
      return sendRequest(`${URL}/agent/${dsid}`, 'POST');
    },
    insert(values) {
        console.log(JSON.stringify(values,false,4))
        values.parentID = dsid;
...
//************************************************ */
  const grid = $('#profileContainer').dxDataGrid({
    ...
    editing: {
      ...
      mode: 'popup',
      allowUpdating: (($('#groupid').text()>1) && ((roleID==4) || (roleID==5) || ((divisionID==1) && (roleID==1))))?true:false,
      allowAdding: (($('#groupid').text()>1) && ((roleID==4) || (roleID==5) || ((divisionID==1) && (roleID==1))))?true:false,
      allowDeleting: (($('#groupid').text()>1) && ((roleID==4) || (roleID==5) || ((divisionID==1) && (roleID==1))))?true:false,
      useIcons: true,
      popup: {
        title: formatMessage('agent'),
        showTitle: true,
        width: 1280,
        height: 840,
      },
    ...
    columns: [
      ...
      { dataField : 'status',
        ...
        visible: true,
        allowEditing: (($('#groupid').text()>1) && ((roleID==4)||(roleID==5)))?true:false, 
        width: 90,
        lookup: {
        ...
        { text: formatMessage('product'),
            ...
            onClick: function (e) {
                if ((roleID == 1) || (roleID==6)) {
                  window.location = `/walletxchange_${e.row.data.memberID}`;
                } else {
    ...
    onRowDblClick(e) {
        if (e.key !== 0) {
            ...
            if (roleID == 1) window.location = `/agestaff_${e.data.memberID}`;
        }
    }
