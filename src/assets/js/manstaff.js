...
$(() => {
  ...
  //****** label header ******/
  $('#pagetitle').html(`${formatMessage('manstaff')}`);
  //**************************/
  ...
  const memberStore = new DevExpress.data.CustomStore({
    ...
    load(loadOptions) {
      ...
      return sendRequestARG(`${URL}/manstaff`, 'POST', args);
    },
    insert(values) {
        console.log(JSON.stringify(values,false,4))
        values.parentID = mid;
        values.groupID = parentGroupID;
        values.company = $('#manufacturerName').text();
        values.roleID = 2;
    ...
    if (typeof cellInfo.value === 'undefined') {
      ...
      if ((typeof cellInfo.value === 'undefined') || (cellInfo.value === null)) {
        ...
        new QRCode(qrcodeElement, {
          text: `${canssmsWebUrlave}/${cellInfo.value}`,
          width: 130,
          height: 130,
          colorDark : "#000000",
          colorLight : "#ffffff",
