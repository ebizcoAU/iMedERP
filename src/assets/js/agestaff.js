...
$(() => {
  //****** label header ******/
  $('#pagetitle').html(`${formatMessage('agestaff')}`);
  //**************************/
  var parentGroupID = $('#agentgroupID').text();
  ...
  const grid = $('#profileContainer').dxDataGrid({
    ...
    columns: [
      ...
      { dataField : 'divisionID',
        ...
        visible: true,
        lookup: {
            dataSource: agentDivisionSource,
            valueExpr: "status",
            displayExpr: "statext"
        }
      },
      ...
      { dataField: 'avgrating',
        caption: formatMessage('ratingavg'),
        width: 80,
        allowEditing: false ,
        format: {
          type: "fixedPoint",
      ...
      { dataField : 'status',
        ...
        visible: true,
        allowEditing: (($('#groupid').text()>1) && (roleID==6))?true:false, 
        width: 90,
        lookup: {
      ...
      { type: 'buttons',
        ...
        { text: formatMessage('product'),
            ...
            hint: "Account",
            onClick: function (e) {
                window.location = `/salexchg_0_0_${e.row.data.memberID}`;
            }
        },{ 
          text: formatMessage('starrating'),
          icon: `favorites`,
          hint: "Star",
          onClick: function (e) {
              if ((roleID == 1) || (roleID==6) ||  (roleID==7)) {
                window.location = `/walletstar_${e.row.data.memberID}`;
              } else {
  ...
  function cellTemplate(container, options) {
    ...
    imgElement.setAttribute("height", `78`);
