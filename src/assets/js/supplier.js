...
$(() => {
  //****** label header ******/
  $('#pagetitle').html(`${formatMessage('supplier')}`);
  //**************************/
  ...
  const memberStore = new DevExpress.data.CustomStore({
    ...
    load(loadOptions) {
        ...
        //console.log(args)
      return sendRequestARG(`${URL}/supplier`, 'POST', args);
    },
    insert(values) {
  ...
  const grid = $('#profileContainer').dxDataGrid({
    ...
    columns: [
      ...
      { dataField : 'divisionID',
        ...
        width: 90,
        lookup: {
            dataSource: supplierDivisionSource,
            valueExpr: "status",
            displayExpr: "statext"
        }
      },
    ...
    onRowDblClick(e) {
        if (e.key !== 0) {
            ...
            if ((roleID == 1) || (roleID==6)) window.location = `/agestaff_${e.data.memberID}`;
        }
    }
  ...
  function cellTemplate(container, options) {
    ...
    imgElement.setAttribute("width", `65`);
    imgElement.setAttribute("height", `65`);
