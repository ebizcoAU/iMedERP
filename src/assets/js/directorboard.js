...
$(() => {
  //****** label header ******/
  $('#pagetitle').html(formatMessage('directorboard'));
  //**************************/
  ...
  const memberStore = new DevExpress.data.CustomStore({
    ...
    load(loadOptions) {
      ...
      return sendRequestARG(`${URL}/manufacturerlist`, 'POST', args);
    },
    insert(values) {
  ...
  const grid = $('#profileContainer').dxDataGrid({
    ...
    editing: {
      ...
      useIcons: true,
      popup: {
        title: formatMessage('directorboard'),
        showTitle: true,
        width: 1280,
        height: 840,
      },
    ...
    columns: [
      { dataField: 'imgLinkx',
        caption: formatMessage('logo') + " 160x190",
        width: 95,
        allowSearch: false,
        allowSorting: false,
      ...
      { dataField: 'divisionID',
        ...
        allowEditing: true,
        lookup: {
          dataSource: adminDivisionSource,
          valueExpr: "status",
          displayExpr: "statext",
        }
      },
      ...
      { dataField : 'status',
        ...
        visible: false,
        allowEditing: ((divisionID==1) && (roleID==1))?true:false,
        width: 90,
        lookup: {
    ...
    onRowDblClick(e) {
        if (e.key !== 0) {
            ...
            window.location = `/distributor_${e.data.memberID}`;
        }
    }
